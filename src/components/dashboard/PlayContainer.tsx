...
export default function PlayContainer(props: PlayContainerProps) {
  ...
  const getStageOrder = () => {
    const order: PlayContainerProps['matchUpStage'][] = []
    if (playingQuiz) order.push('quiz')
    if (playingMatchUp) order.push('lines')
    if (playingGroupSort) order.push('group_sort')
    if (playingFindTheMatch) order.push('find_the_match')
    if (playingOpenBox) order.push('open_box')
    if (playingAnagram) order.push('anagram')
    if (playingDebate) order.push('debate')
    return order
  }
  const nextStageAfter = (current: PlayContainerProps['matchUpStage'] | 'study' | 'mnemonic_practice' | null): PlayContainerProps['matchUpStage'] => {
    const order = getStageOrder()
    if (order.length === 0) return 'summary'
    if (!current || current === 'study' || current === 'mnemonic_practice') return order[0]
    const idx = order.indexOf(current)
    return (idx >= 0 && idx < order.length - 1) ? order[idx + 1] : 'summary'
  }
  return (
        ...
        <CardBody>
          {matchUpStage === 'study' && playingStudyElements && playingStudyElements.length > 0 && (
            <StudyStage
              ...
              nextStageAfterStudy={(() => {
                const order = getStageOrder()
                const next = order[0] || 'summary'
                return next as 'quiz' | 'lines' | 'group_sort' | 'find_the_match' | 'open_box' | 'anagram' | 'debate' | 'summary'
              })()}
          ...
          {matchUpStage === 'quiz' && playingQuiz && (
            <QuizStage
              ...
              continueLabel={(() => {
                const next = nextStageAfter('quiz')
                return next === 'summary' ? 'Finalizar' : 'Continuar'
              })()}
              onComplete={(result) => {
              ...
              onContinue={() => {
                const next = nextStageAfter('quiz')
                setMatchUpStage(next)
                if (user?.id && playingResourceId) {
                  if (next === 'summary') {
                    saveResourceProgress(user.id, playingResourceId, { stage: 'summary', quizConfirmed: true })
                  } else {
                    saveResourceProgress(user.id, playingResourceId, { stage: next as ResourceStage, quizConfirmed: true })
                  }
                }
              }}
            />
          ...
          {matchUpStage === 'mnemonic_practice' && playingStudyElements && playingStudyElements[studyIndex]?.type === 'mnemonic_creator' && (
            <MnemonicPracticeStage
              ...
              onContinue={() => {
                const nextStudyIndex = studyIndex + 1
                const hasMoreStudy = Array.isArray(playingStudyElements) && nextStudyIndex < (playingStudyElements?.length || 0)
                if (hasMoreStudy) {
                  setMatchUpStage('study')
                  setStudyIndex(nextStudyIndex)
                  setStudyItemCompleted(false)
                  if (user?.id && playingResourceId) {
                    saveResourceProgress(user.id, playingResourceId, { stage: 'study', studyIndex: nextStudyIndex, studyItemCompleted: false, mnemonicPracticeConfirmed: true })
                  }
                } else {
                  const next = nextStageAfter(null)
                  setMatchUpStage(next)
                  if (user?.id && playingResourceId) {
                    saveResourceProgress(user.id, playingResourceId, { stage: (next === 'summary' ? 'summary' : next as ResourceStage), mnemonicPracticeConfirmed: true })
                  }
                }
              }}
            />
